...
## 地の文から除外されるもの（全検出器の共通規約）

抽出は `prose.rs` の単一 home が持ち、全検出器がそこから散文単位・文を受け取る。次は地の文でないので、検査の対象にも文長・密度の分母にもならない。

- **code fence**（``` で囲む範囲）と **inline code**（`` ` `` で囲む範囲） — コードは散文でなく、語は「言及」である。
- **display 数式**（`$$ … $$`。複数行でも 1 行でも同じ） — 中身は LaTeX ソースであって文でない。
- **表**（`|` で始まる行）と**見出し**（`#` で始まる行） — 構造であって地の文でない。
- **URL**・**§参照**・**台帳 ID**（`R2607_001`・`IF-3` 等） — 文中から取り除く。

いずれも**同数の改行に置換**して除去するため、finding の行番号は常に原文の行を指す。

inline 数式（`$ … $`）は除外**しない**。通貨表記や shell 変数と衝突するうえ、一文の長さを壊すのは display のほうだけだからである。

display 数式を除外する経緯（2026-07-28 の実測）は次の通り。103 字の LaTeX ソースが sentence-length に「一文 103 字」として当たった。空行で段落を切ると区切りの記号まで数えて 107 字に増える。数式を fence へ移せば黙るが、数式として描画されなくなる。記法そのものは標準として動かさず、correo のほうで除外する道を採った。

対の判定には二つの安全弁がある。inline code の中にある記号は記法への言及とみなし、区切りには数えない（本節の説明文がまさにこれである）。空行を跨ぐ対も数式でないと判じて素通しする。誤った対は地の文を黙って飲み込む — lint にとって最悪の壊れ方は、沈黙する見落としだからである。

## 低レベル検出器の出力

`codemix` は密度が閾値を超えた段落を `⚑ L{行}: N latin/100字` で列挙（常に exit 0・advisory）。`coinage --strict` は辞書外複合を候補列挙し、`--advisory` 無しなら違反時 exit 1。

**codemix の判定条件**: JA 40字未満の段落は判定しない（密度が統計的に無意味なため）。ただし**連続する箇条書きは 1 まとまりに集計**して条件を越えさせる（LLM slop は短い bullet に湧くため。空行区切りの bullet の列も 1 単位）。code fence・display 数式・inline code・URL・表・見出し行は地の文から除外して数える（共通規約は前節）。短い段落に紛れる混種語・造語は coinage が段落長と無関係に捕まえる。
