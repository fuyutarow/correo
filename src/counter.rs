// counter.rs — 助数詞欠落の検出（2026-07-11・台帳体プロジェクトの句レベル実務指摘）。
//
...
//
// 較正（2026-07-11・台帳体の実務文書から読者向け文書へ複写された HTML 抽出 corpus）: 「数字 +
// 空白 + ラテン語幹」の直後に日本語助詞（を・が・は・で・に・と・へ・の）または句読点が続く形を
// 機械抽出したところ、陽性は「43 cell を」「8 cell の」「10 track を」の 3 件のみだった。
// 同コーパスには「43 cell（7 家族群別）」「13 cell ── PASS」のようにダッシュ・括弧・改行へ
// 続く非陽性が 27 件あり、これらは助詞・句読点に直結しないため対象外のままでよい
// （規則の狙いどおり）。correo 自身の README.md 他の *.md では該当ゼロ（陰性の再確認）。
//
// 除外（見送り理由を明記する規約に従う）:
...
/// ここに載せるのは SI・計算機の標準単位と JA（README §counter が既定として文書化済み）だけ —
/// ドメイン固有の単位（QoI 等・2026-07-11 外部化で組み込みから撤去）は利用側が correo.toml の
/// `[counter] unit-words` で持ち込む（house 語彙を組み込みに焼き込まない deny-vocabulary の規約）。
const UNIT_WORDS: &[&str] = &[
    "ms", "s", "sec", "min", "h", "kg", "g", "mg", "km", "cm", "mm", "nm", "pt", "px", "dB", "Hz",
    "kHz", "MHz", "GHz", "bit", "bits", "byte", "bytes", "JA",
];

/// 文書 → 助数詞欠落の全 finding（fence/inline code/表/見出し/URL を除いた地の文が対象）。
/// 除外は prose.rs の clean_lines を共有する（notation.rs と同じ規約）。extra_units は
/// `[counter] unit-words` 由来の追加単位語（組み込み UNIT_WORDS との union・一致比較）。
pub fn scan(text: &str, extra_units: &std::collections::HashSet<String>) -> Vec<Finding> {
    let mut out = Vec::new();
    ...
    for (i, raw) in defenced.lines().enumerate() {
        ...
        for caps in hit.captures_iter(raw) {
            ...
            if UNIT_WORDS.contains(&word) || extra_units.contains(word) {
                continue;
            }
...
mod tests {
    ...
    fn no_extra() -> std::collections::HashSet<String> {
    ...
    fn flags_number_latin_directly_followed_by_particle_wo() {
        ...
        let hits: Vec<_> = scan(text, &no_extra())
            .into_iter()
    ...
    fn table_rows_and_inline_code_are_excluded_like_other_detectors() {
        ...
        assert!(
            scan(table_row, &no_extra())
                .iter()
                .all(|f| f.rule != "missing-counter"),
            "表の行は地の文として検査してはいけない"
        );
    }
    ...
    fn calibration_corpus_representative_sample_matches_incident_counts() {
        // 実測較正（2026-07-11・台帳体の実務文書から読者向け文書へ複写された HTML 抽出 corpus）:
        // 陽性は
    ...
    #[test]
    fn extra_units_from_config_silence_domain_unit_words() {
        // QoI はドメイン単位（組み込みから撤去済み）— 素では発火し、`[counter] unit-words`
        // で持ち込めば沈黙する（house 語彙は利用側、の規約の回帰）。
        let text = "推定誤差は 3 QoI を下回る。";
        assert!(
            scan(text, &no_extra())
                .iter()
                .any(|f| f.rule == "missing-counter"),
            "組み込みから撤去した QoI が素で沈黙している"
        );
        let extra: std::collections::HashSet<String> = ["QoI".to_string()].into();
        assert!(
            scan(text, &extra).is_empty(),
            "unit-words で持ち込んだ単位語が発火した: {:?}",
            scan(text, &extra)
                .iter()
                .map(|f| &f.msg)
                .collect::<Vec<_>>()
