...
pub fn scan(text: &str) -> Vec<Violation> {
    ...
    // 反証の採録（2026-07-17・coji/natural-japanese の 7 モデル×406 本実測）: 文頭反復の文書
    // 発火率は人間 93% vs AI 41% と人間側が圧倒的に多い（意図的なリズム技法）。総量・比率での
    // 規制は人間の名文を先に撃つ — 本規則が「3 文連続」の狭い形だけを見て、離れた反復を
    // 検査しないのはこの反証と整合する。閾値を緩める・総量規制へ広げる変更は禁止。
