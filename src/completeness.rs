...
// 断片（見出し・ラベル・統計カードの残骸）が「文」として紛れ込む事故が qoed ポートフォリオ
// HTML で実測された（`§1 設計空間 ── campaign E の六軸` 「全 gate 通過・結合 151 assertion」の
// ような、句点も疑問符も持たない断片が段落の位置に落ちる）。検出器 = 検査する性質 1 つ
...
mod tests {
    ...
    fn html_derived_fragment_regression() {
        // qoed ポートフォリオ HTML 抽出で実測した断片型（統計カードの残骸）。
